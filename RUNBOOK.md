...
| Braintrust deploy auth wiring | ✅ Done | `BraintrustLogger` now accepts explicit `api_key`, and FastAPI lifespan passes `settings.braintrust_api_key` so deployed containers log to Braintrust even when env-to-SDK auto-discovery fails. |
...
| Test env isolation (avatar_provider) | ✅ Done | `test_server.py` autouse fixture pins `avatar_provider=simli` so WS protocol tests pass regardless of local `.env` avatar setting. 373 backend + 169 frontend tests green. |
| SpatialReal API key rotation | ✅ Done | Updated local `.env` and Fly.io secret; verified via token endpoint (200). |
| Production redeployment (2026-03-26) | ✅ Done | `flyctl deploy --remote-only` — both machines healthy; https://nerdy-tutor.fly.dev/ returns 200. |
| Spacebar hint tooltip (mic button) | ✅ Done | Native `title` tooltip shows "Hold Space to talk" on hover; state-aware for recording/disabled |
